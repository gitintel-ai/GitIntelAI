...
└────────────┘               └──────────────┬───────────────┘
      ...
      <p>
        Your AI coding agent calls <code>gitintel checkpoint</code> before writing each file,
        recording which lines it wrote and which model/session produced them. When you run{" "}
        <code>git commit</code>, the post-commit hook automatically assembles these checkpoints into
        an Authorship Log and stores it in git notes — a standard git mechanism that travels with
        your repo on every push and fetch.
      </p>
      ...
      <div className="not-prose space-y-6 mt-4">
        <div className="relative pl-6 before:absolute before:left-2 before:top-3 before:h-full before:w-px before:bg-border">
          <div className="absolute left-0 top-2 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground">
            1
          </div>
          <div className="rounded-lg border p-4">
            <h3 className="font-semibold">Checkpoint Recording</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              The AI agent (or you manually) calls{" "}
              <code className="rounded bg-muted px-1 font-mono text-xs">gitintel checkpoint</code>{" "}
              with the agent name, model, session ID, file path, and the line ranges it wrote.
              Checkpoints are stored in a local SQLite database and are never committed to git.
            </p>
          </div>
        </div>
        ...
        <div className="relative pl-6 before:absolute before:left-2 before:top-3 before:h-full before:w-px before:bg-border">
          ...
          <div className="rounded-lg border p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              When you run{" "}
              <code className="rounded bg-muted px-1 font-mono text-xs">git commit</code>, the
              post-commit hook automatically executes. It loads all pending checkpoints for the
              current session, verifies the line ranges against the committed diff, and builds an
              Authorship Log.
            </p>
          </div>
        </div>
        ...
        <div className="relative pl-6 before:absolute before:left-2 before:top-3 before:h-full before:w-px before:bg-border">
          ...
          <div className="rounded-lg border p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              The hook writes YAML attribution data to{" "}
              <code className="rounded bg-muted px-1 font-mono text-xs">
                refs/ai/authorship/&lt;commit-sha&gt;
              </code>{" "}
              using standard git notes. Any lines not covered by a checkpoint are attributed as
              Human.
            </p>
            <div className="mt-3 rounded bg-muted p-3 font-mono text-xs">
              <p className="text-muted-foreground">
                # Written automatically by the post-commit hook:
              </p>
        ...
        <div className="relative pl-6">
          ...
          <div className="rounded-lg border p-4">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Git notes sync alongside your code when you configure your remote. Team members
              running
      ...
      <p>
        Attribution data is stored as YAML in git notes at{" "}
        <code>refs/ai/authorship/&lt;sha&gt;</code>. This is an open standard — any tool can read it
        using plain <code>git notes</code>. No GitIntel installation is required to consume the
        data.
      </p>
...
summary:
  ...
  primary_model: "claude-sonnet-4-6"`}</pre>
      ...
      <p>
        Claude Code exports native OpenTelemetry metrics. GitIntel runs a local OTel collector on
        port 4317 that captures cost and token data automatically — no manual checkpoints required
        for cost tracking.
      </p>
      ...
      <div className="not-prose grid gap-4 sm:grid-cols-2 mt-4">
        <div className="rounded-lg border p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            All tracking data stored at <code className="font-mono">~/.gitintel/gitintel.db</code>
          </p>
          <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
            <li>
              • <code className="font-mono">checkpoints</code> — pending AI session data
            </li>
            <li>
              • <code className="font-mono">attributions</code> — committed attribution records
            </li>
            <li>
              • <code className="font-mono">cost_sessions</code> — OTel cost data
            </li>
            <li>
              • <code className="font-mono">memory</code> — key-value memory store
            </li>
          </ul>
        </div>
        <div className="rounded-lg border p-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Attribution YAML stored in standard git notes
          </p>
          <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
            <li>
              • Stored at <code className="font-mono">refs/ai/authorship/&lt;sha&gt;</code>
            </li>
            <li>
              • Syncs via <code className="font-mono">git push/fetch</code>
            </li>
