...
import { db, budgetAlerts, organizations } from "../db";
...
alertRoutes.get("/budget", async (c) => {
  ...
  try {
    ...
    const isUuid = auth.orgId && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(auth.orgId);
    const results = isUuid
      ? await db.select().from(budgetAlerts).where(eq(budgetAlerts.orgId, auth.orgId!))
...
alertRoutes.put(
  "/budget/:id",
  zValidator("json", createAlertSchema.partial()),
  async (c) => {
